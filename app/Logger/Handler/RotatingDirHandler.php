...
class RotatingDirHandler extends StreamHandler

{
    public const DIR_PER_DAY = 'Y-m-d';
    public const DIR_PER_MONTH = 'Y-m';
    public const DIR_PER_YEAR = 'Y';
    ...
    public function __construct(string $filename, int $maxFiles = 0, $level = Logger::DEBUG, bool $bubble = true, ?int $filePermission = null, bool $useLocking = false)
    {
        ...
        $this->dateFormat = static::DIR_PER_DAY;
    ...
    public function setFilenameFormat(string $filenameFormat, string $dateFormat): self
    {
        if (!preg_match('{^[Yy](([/_.-]?m)([/_.-]?d)?)?$}', $dateFormat)) {
            throw new InvalidArgumentException(
                ...
                'RotatingFileHandler::DIR_PER_DAY ("Y-m-d"), RotatingFileHandler::DIR_PER_MONTH ("Y-m") ' .
                'or RotatingFileHandler::DIR_PER_YEAR ("Y"), or you can set one of the ' .
    ...
    protected function getTimedFilename(): string
    {
        ...
        $timedFilename = str_replace(
            ...
            $fileInfo['dirname'] . '/{date}/' . $this->filenameFormat
        );
